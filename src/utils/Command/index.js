...
const Command = class {
	 ...
	 * @param {String} [options.category='Miscellaneous'] - Base role for users that will ignore cooldown
	 */
	constructor(options = {}) {
	...
	getUsage(prefix) {
		if (typeof this.usage === 'object')
			return `${prefix}${this.name} ${this.usage[0]}`;

		return `${prefix}${this.name} ${this.usage}`;
	}

	getPartialUsage() {
		if (typeof this.usage === 'object') return `${this.usage[0]}`;
		return `${this.usage}`;
	}

	getFullUsage(prefix) {
		if (typeof this.usage === 'object') {
			return this.usage.map(usage => `${prefix}${this.name} ${usage}`).join('\n');
		}
		return `${prefix}${this.name} ${this.usage}`;
	}
